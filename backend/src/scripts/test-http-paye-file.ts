...
import { PayeReturnSubmitter } from '../workers/http/filing/PayeReturnSubmitter';
...
async function main() {
    ...
    const payeZipPath = process.argv[6] ?? '';

    if (!kraPin || !kraPassword || !payeZipPath) {
        console.error('Usage: npx ts-node src/scripts/test-http-paye-file.ts <pin> <password> [periodFrom] [periodTo] <payeZipPath>');
        process.exit(1);
    }

    const jobId = `test-http-paye-${Date.now()}`;
    const job = {
        id: jobId,
        data: { jobId, userId: 'test-user', payload: { kraPin, taxObligationType: 'paye', periodFrom, periodTo, payeZipUrl: payeZipPath, clientId: 'test-client' } },
        progress: 0,
    ...
    try {
        ...
        await navigator.selectReturnObligation('paye', kraPin);

        console.log('--- Step 3: Upload PAYE return ---');
        const submitter = new PayeReturnSubmitter(session, job);
        const result = await submitter.execute({ kraPin, periodFrom, periodTo, payeZipUrl: payeZipPath });

        console.log('\n=== PAYE Filing Result ===');
        console.log('Receipt Number:', result.receiptNumber);
        console.log('Receipt Path:', result.receiptPath);
    } catch (err: any) {
        console.error('PAYE filing failed:', err.message);
        if (err.rawResponse) {
