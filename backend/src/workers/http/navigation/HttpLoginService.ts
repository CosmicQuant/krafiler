...
export class HttpLoginService {
    ...
    async execute(kraPin: string, kraPassword: string, otpCode?: string): Promise<HttpLoginResult> {
        ...
        await appendJobLog(this.job, `CAPTCHA solved via Gemma 4: ${captchaAnswer}`, { progress: 25 });
        ...
        if (outcome.type === 'failure') {
            ...
            throw new KraError(KraErrorCode.CREDENTIALS_INVALID, combined || 'Login failed', { rawResponse: loginResponse.slice(0, 2000) });
        }
