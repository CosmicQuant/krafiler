...
router.get('/:clientId/leave-types', async (req, res) => {
    try {
...
router.post('/:clientId/leave-types', async (req, res) => {
    try {
        const clientId = parseInt(req.params.clientId, 10);
        if (isNaN(clientId)) return res.status(400).json({ message: 'Invalid client ID' });
...
router.put('/:clientId/leave-types/:id', async (req, res) => {
    try {
        ...
        await db
            .updateTable('leave_types')
            .set({
                name: name !== undefined ? name : existing.name,
                isPaid: isPaid !== undefined ? (isPaid === false || isPaid === 0 ? 0 : 1) : existing.isPaid,
                maxDaysPerYear: maxDaysPerYear !== undefined ? maxDaysPerYear : existing.maxDaysPerYear,
...
router.delete('/:clientId/leave-types/:id', async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        const clientId = parseInt(req.params.clientId, 10);
        if (isNaN(id) || isNaN(clientId)) return res.status(400).json({ message: 'Invalid ID' });
