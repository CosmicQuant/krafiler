...
if (!code.includes('const handleAutoFile')) {
...
`;
    // Insert after handleGenerateAllZips
    code = code.replace(/const handleGenerateAllZips = async \(\) => \{[\s\S]*?setIsGeneratingZips\(false\);\n\s+\};\n?/, '$&\n' + handleAutoFileFn);

    // Update the button onClick to handleAutoFile    
    code = code.replace(/onClick=\{\(\) => setDashboardNotice\(\{ tone: 'info', message: `Auto-filing scheduled for \$\{client.name\}.` \}\)\}\n\s+disabled=\{!client.payeZipUrl && !client.nssfFileUrl && !client.shaFileUrl\}/,
    ...
    console.log('Frontend dashboard patched!');
}
