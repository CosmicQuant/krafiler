...
export function LoanManager({ clientId }: LoanManagerProps) {
    ...
    const openModal = (rec?: Loan) => {
        if (rec) {
            ...
            setForm({
                ...
                kraPin: rec.kraPin || '', loanType: rec.loanType || 'Salary Advance',
                principal: rec.principal || 0, monthlyDeduction: rec.monthlyDeduction || 0,
                installments: rec.installments || 1, remainingInstallments: rec.remainingInstallments || 1,
                interestRate: rec.interestRate || 0, totalInterest: rec.totalInterest || 0,
                totalRepayable: rec.totalRepayable || 0, amountPaid: rec.amountPaid || 0,
                status: rec.status || 'Approved', disbursedAt: rec.disbursedAt || '',
                notes: rec.notes || '',
            });
        } else {
            setEditing(null);
            ...
            {loading ? (
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                        ...
                        <tbody>
                            {loans.map((ln) => (
                                <tr key={ln.id} className="border-b border-slate-50 hover:bg-slate-50">
                                    ...
                                    <td className="py-2 text-right">
                                        <div className="flex items-center justify-end gap-1">
                                            <button onClick={() => openModal(ln)} className="p-1 rounded hover:bg-blue-50 text-blue-600 transition"><Pencil className="h-3.5 w-3.5" /></button>
                                            <button onClick={() => handleDelete(ln.id)} className="p-1 rounded hover:bg-red-50 text-red-500 transition"><Trash2 className="h-3.5 w-3.5" /></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
                    ...
                    <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl bg-white shadow-xl">
                        ...
                        <div className="px-6 py-4 space-y-3">
                            <div className="grid grid-cols-2 gap-3">
                                <div><label className="mb-1 block text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Employee ID</label><input type="number" value={form.employeeId} onChange={e => setForm((f: any) => ({ ...f, employeeId: e.target.value }))} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-900 focus:border-slate-400 focus:outline-none focus:ring-1 focus:ring-slate-400" /></div>
                                <div><label className="mb-1 block text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Employee Name</label><input type="text" value={form.employeeName} onChange={e => setForm((f: any) => ({ ...f, employeeName: e.target.value }))} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-900 focus:border-slate-400 focus:outline-none focus:ring-1 focus:ring-slate-400" /></div>
                                <div><label className="mb-1 block text-[10px] font-semibold text-slate-500 uppercase tracking-wider">KRA PIN</label><input type="text" value={form.kraPin} onChange={e => setForm((f: any) => ({ ...f, kraPin: e.target.value }))} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-900 focus:border-slate-400 focus:outline-none focus:ring-1 focus:ring-slate-400" /></div>
