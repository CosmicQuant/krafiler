...
export function getTotalScheduledHours(config: WorkScheduleConfig, period: string): number {
    if (!config || Object.keys(config).length === 0) return 240;
...
/**
 * Thin wrapper that matches the backend /payroll/calculate-preview response shape.
 */
export function calculatePayrollPreview(
    ...
    const computed = computePayrollEntry(input, period, prorate, workScheduleConfig, holidays, adjustments);
    return {
        grossPay: computed.grossPay,
        shaDeduction: computed.shaDeduction,
        nssfDeduction: computed.nssfDeduction,
        ahlDeduction: computed.ahlDeduction,
